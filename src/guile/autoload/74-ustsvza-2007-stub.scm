;; Copyright(C) 2004,05,06 Stefan Siegl <stesie@brokenpipe.de>
...
(and 
 (require-version 0 4 "Antrag auf Dauerfristverlängerung 2007")

 ;; if version requirement is met, try to add the sheet ...
 (tb:form-register
  ;; form's name
  "Antrag auf Dauerfristverlängerung 2007" 
  ...
  (lambda (buf test)
	...
	(let ((sig-result (and (not test)
			       (tb:check-sig "signatures/ustsvza-2007.sig"))))
	  ...
	  (export:make-elster-xml
	   ...
	   (ustsvza-2007:export-steuerfall buf sig-result)))))
