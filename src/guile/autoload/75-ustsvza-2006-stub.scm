...
(and 
 (require-version 0 4 "Antrag auf Dauerfristverlängerung 2006")

 ;; if version requirement is met, try to add the sheet ...
 (tb:form-register
  ;; form's name
  "Antrag auf Dauerfristverlängerung 2006" 

  ;; get sheet tree -----------------------------------------------------------
  (lambda ()
    (tb:eval-file "ustsvza-2006.scm")
    (tb:eval-file "sheettree.scm")
    (extract-sheet-tree ustsvza-2006:definition))
  ...
  (lambda (sheetname)
    (tb:eval-file "ustsvza-2006.scm")
    (ustsvza-2006:get-sheet sheetname))
  ...
  (lambda (buffer element value)
	...
	(let ((validity (and (ustsvza-2006:validate buffer element value)
	  ...
	  (if validity
		...
		(if (string=? element "ustvz")
		    (ustsvza-2006:recalculate buffer element value))))
  ...
  (lambda (buf test)
    ...
    (if 
     (and (revalidate:buffer ustsvza-2006:validate buf ustsvza-2006:validators)
	...
	(let ((sig-result (and (not test)
	  ...
	  (export:make-elster-xml
	   ...
	   (ustsvza-2006:export-steuerfall buf sig-result)))))
